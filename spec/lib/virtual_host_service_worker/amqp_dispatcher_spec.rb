  ...
  describe '.push_reload_to_amqp' do
    ...
    before do
      stub_const('APP_CONFIG', {
                   ...
                   'haproxy_command' => File.expand_path('../../support/haproxy_dummy', __dir__),
                   'haproxy_reload' => File.expand_path('../../support/haproxy_dummy', __dir__),
    ...
    it 'reqeus when haproxy instance limit reached' do
      ...
      described_class.dispatch(valid_payload)
    end

    it 'Execute reload when retrying queued item' do
      ...
      described_class.dispatch(reload_payload)
    end
  end
end
