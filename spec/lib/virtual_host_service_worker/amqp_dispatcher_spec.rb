...
RSpec.describe VirtualHostServiceWorker::AmqpDispatcher do
  describe '.push_reload_to_amqp' do
    ...
    it 'reqeus when haproxy instance limit reached' do
      ...
      allow(VirtualHostServiceWorker).to receive(:setup_v_host)
      # allow(VirtualHostServiceWorker).to receive(:currentInstances)
      allow(VirtualHostServiceWorker).to receive(:haproxy_instance_limit_reached?).with(:currentInstances).and_return true
