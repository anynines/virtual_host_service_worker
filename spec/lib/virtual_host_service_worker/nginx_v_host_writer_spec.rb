require 'spec_helper'

describe VirtualHostServiceWorker::NginxVHostWriter do
  let :valid_payload do
    {
      'id' => 1,
      'ssl_certificate' =>
      "-----BEGIN CERTIFICATE-----\\n7BeVZ1Q+foNw3Yl5o46J5K5AvIulzMCP\n-----END CERTIFICATE-----",
      'ssl_ca_certificate' =>
      "-----BEGIN CERTIFICATE-----\\nNZbleCAVFndhuSHuLxU27IhCvh/zb9XcBfMLaH7ul03XO7SHqzASCxwZ\n-----END CERTIFICATE-----",
      'ssl_key' =>
      "-----BEGIN RSA PRIVATE KEY-----\\nDVyMxqPfW1XvQj5Vx2naUio7gy3zg5Y+v7+LbJY2g7Cc\n-----END RSA PRIVATE KEY-----",
      'server_name' => 'eXample.de',
      'organization_guid' => 'a-valid-org-guid',
      'created_at' => '2013-07-03T07:52:05Z',
      'updated_at' => '2013-07-03T07:52:05Z'
    }
  end

  let :valid_payload_with_wildcard_server_name do
    {
      'id' => 1,
      'ssl_certificate' =>
      "-----BEGIN CERTIFICATE-----\\n7BeVZ1Q+foNw3Yl5o46J5K5AvIulzMCP\n-----END CERTIFICATE-----",
      'ssl_ca_certificate' =>
      "-----BEGIN CERTIFICATE-----\\nNZbleCAVFndhuSHuLxU27IhCvh/zb9XcBfMLaH7ul03XO7SHqzASCxwZ\n-----END CERTIFICATE-----",
      'ssl_key' =>
      "-----BEGIN RSA PRIVATE KEY-----\\nDVyMxqPfW1XvQj5Vx2naUio7gy3zg5Y+v7+LbJY2g7Cc\n-----END RSA PRIVATE KEY-----",
      'server_name' => '*.eXample.de',
      'organization_guid' => 'a-valid-org-guid',
      'created_at' => '2013-07-03T07:52:05Z',
      'updated_at' => '2013-07-03T07:52:05Z'
    }
  end

  let :valid_payload_with_empty_ca_cert do
    {
      'id' => 1,
      'ssl_certificate' =>
      "-----BEGIN CERTIFICATE-----\\n7BeVZ1Q+foNw3Yl5o46J5K5AvIulzMCP\n-----END CERTIFICATE-----",
      'ssl_ca_certificate' =>
      nil,
      'ssl_key' =>
      "-----BEGIN RSA PRIVATE KEY-----\\nDVyMxqPfW1XvQj5Vx2naUio7gy3zg5Y+v7+LbJY2g7Cc\n-----END RSA PRIVATE KEY-----",
      'server_name' => '*.eXample.de',
      'organization_guid' => 'a-valid-org-guid',
      'created_at' => '2013-07-03T07:52:05Z',
      'updated_at' => '2013-07-03T07:52:05Z'
    }
  end

  let :valid_nginx_config do
    <<-END_OF_CONFIG
    error_log  logs/error.log;
    pid        logs/nginx.pid;
    worker_rlimit_nofile 8192;
    #{' '}
    events {
      worker_connections  4096;
    }

    http {
      index    index.html index.htm index.php;
    }
    END_OF_CONFIG
  end

  describe '.setup_v_host' do
    before do
      `rm -f #{APP_CONFIG['cert_dir']}example.de/example.de.pem`
      `rm -f #{APP_CONFIG['cert_dir']}example.de/example.de.key`
      `rm -f #{APP_CONFIG['v_host_config_dir']}example.de.conf`
      `rm -f #{APP_CONFIG['v_host_link_dir']}example.de.conf` if APP_CONFIG['v_host_link_dir']
      `rm -f #{APP_CONFIG['v_host_link_dir']}wild.example.de.conf` if APP_CONFIG['v_host_link_dir']
    end

    context 'with a valid cert, ca cert and ssl key' do
      it 'creates the key file' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
        expect(File.exist?("#{APP_CONFIG['v_host_config_dir']}/example.de.conf")).to be true
      end

      it 'creates the cert key file' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
        expect(File.exist?("#{APP_CONFIG['cert_dir']}/example.de/example.de.key")).to be true
      end

      it 'creates the cert pem file' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
        expect(File.exist?("#{APP_CONFIG['cert_dir']}/example.de/example.de.pem")).to be true
      end

      it 'reloads the nginx configuration' do
        expect(VirtualHostServiceWorker::NginxVHostWriter).to receive(:reload_config).once
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
      end

      it 'links the nginx configuration to the directory specified in the config' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
        expect(File.exist?("#{APP_CONFIG['v_host_link_dir']}/example.de.conf")).to be true
      end

      it 'writes the right pem certificate file path to the nginx configuration' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)

        nginx_config_file_content = File.read("#{APP_CONFIG['v_host_link_dir']}/example.de.conf")
        expect(nginx_config_file_content).to include "#{APP_CONFIG['cert_dir']}example.de/example.de.pem"
      end

      it 'writes the right pem certificate file path to the nginx configuration' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)

        nginx_config_file_content = File.read("#{APP_CONFIG['v_host_link_dir']}/example.de.conf")
        expect(nginx_config_file_content).to include "#{APP_CONFIG['cert_dir']}example.de/example.de.key"
      end
    end

    context 'without a ca certificate' do
      it 'creates the cert pem file' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload_with_empty_ca_cert)
        expect(File.exist?("#{APP_CONFIG['cert_dir']}/wild.example.de/wild.example.de.pem")).to be true
      end
    end

    context 'with a wildcard server name' do
      it 'replaces the asterix in the certificates file name' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload_with_wildcard_server_name)
        expect(File.exist?("#{APP_CONFIG['cert_dir']}/wild.example.de/wild.example.de.pem")).to be true
      end

      it 'replaces the asterix in the key file name' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload_with_wildcard_server_name)
        expect(File.exist?("#{APP_CONFIG['cert_dir']}/wild.example.de/wild.example.de.key")).to be true
      end

      it 'replaces the asterix in the server config file' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload_with_wildcard_server_name)
        expect(File.exist?("#{APP_CONFIG['v_host_config_dir']}/wild.example.de.conf")).to be true
      end

      it 'writes the right pem certificate file path to the nginx configuration' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload_with_wildcard_server_name)

        nginx_config_file_content = File.read("#{APP_CONFIG['v_host_link_dir']}wild.example.de.conf")
        expect(nginx_config_file_content).to include "#{APP_CONFIG['cert_dir']}wild.example.de/wild.example.de.pem"
      end

      it 'writes the right pem certificate file path to the nginx configuration' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload_with_wildcard_server_name)

        nginx_config_file_content = File.read("#{APP_CONFIG['v_host_link_dir']}wild.example.de.conf")
        expect(nginx_config_file_content).to include "#{APP_CONFIG['cert_dir']}wild.example.de/wild.example.de.key"
      end

      it 'does not replace the asterisk in the server name for the nginx server name configuration field' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload_with_wildcard_server_name)

        nginx_config_file_content = File.read("#{APP_CONFIG['v_host_link_dir']}wild.example.de.conf")
        expect(nginx_config_file_content).to include 'server_name  *.example.de'
      end
    end

    context 'with a server name already existing in the webserver config' do
      before do
        `mkdir #{APP_CONFIG['cert_dir']}example.de`
        `echo '#{'content_to_override' * 1000}' > #{APP_CONFIG['cert_dir']}example.de/example.de.pem`
        `echo '#{'content_to_override' * 1000}' > #{APP_CONFIG['cert_dir']}example.de/example.de.key`
        `echo '#{'content_to_override' * 1000}' > #{APP_CONFIG['v_host_config_dir']}example.de.conf`
        if APP_CONFIG['v_host_link_dir']
          `echo '#{'content_to_override' * 1000}' > #{APP_CONFIG['v_host_link_dir']}example.de.conf`
        end
        if APP_CONFIG['v_host_link_dir']
          `echo '#{'content_to_override' * 1000}' > #{APP_CONFIG['v_host_link_dir']}wild.example.de.conf`
        end
      end

      it 'finds the existing files (the before each in the test should work)' do
        expect(File.read("#{APP_CONFIG['cert_dir']}example.de/example.de.pem")).to include 'content_to_override'
      end

      it 'replaces the cert pem file' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
        expect(File.read("#{APP_CONFIG['cert_dir']}example.de/example.de.pem")).not_to include 'content_to_override'
      end

      it 'replaces the ssl key' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
        expect(File.read("#{APP_CONFIG['cert_dir']}example.de/example.de.key")).not_to include 'content_to_override'
      end

      it 'replaces the config file' do
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
        expect(File.read("#{APP_CONFIG['v_host_config_dir']}/wild.example.de.conf")).not_to include 'content_to_override'
      end
    end
  end

  describe '.delete_v_host' do
    context 'with a not existing virtual host' do
      it 'does nothing and should not raise an exception' do
        skip
      end
    end

    context 'with a existing virtual host' do
      before do
        `rm -f #{APP_CONFIG['v_host_link_dir']}example.de.conf` if APP_CONFIG['v_host_link_dir']
        VirtualHostServiceWorker::NginxVHostWriter.setup_v_host(valid_payload)
      end

      it 'deletes the vhost config file' do
        VirtualHostServiceWorker::NginxVHostWriter.delete_v_host('eXample.de')
        expect(File.exist?("#{APP_CONFIG['v_host_config_dir']}example.de.conf")).to be false
      end

      it 'deletes the ssl key file' do
        VirtualHostServiceWorker::NginxVHostWriter.delete_v_host('eXample.de')
        expect(File.exist?("#{APP_CONFIG['cert_dir']}example.de/example.de.key")).to be false
      end

      it 'deletes the pem file' do
        VirtualHostServiceWorker::NginxVHostWriter.delete_v_host('eXample.de')
        expect(File.exist?("#{APP_CONFIG['cert_dir']}example.de/example.de.pem")).to be false
      end

      it 'reloads the nginx configuration' do
        expect(VirtualHostServiceWorker::NginxVHostWriter).to receive(:reload_config).once
        VirtualHostServiceWorker::NginxVHostWriter.delete_v_host('eXample.de')
      end

      it 'deletes the config link' do
        VirtualHostServiceWorker::NginxVHostWriter.delete_v_host('eXample.de')
        expect(File.symlink?("#{APP_CONFIG['v_host_link_dir']}example.de.conf")).to be false
      end
    end
  end

  describe '.config_valid?' do
    context 'with a valid config file' do
      before do
        APP_CONFIG['webserver_config'] = "#{APP_CONFIG['v_host_config_dir']}valid_nginx.conf"

        File.write(APP_CONFIG['webserver_config'], valid_nginx_config)
      end

      it 'is true' do
        expect(VirtualHostServiceWorker::NginxVHostWriter.config_valid?).to be true
      end
    end

    context 'with an invalid config file' do
      before do
        APP_CONFIG['webserver_config'] = "#{APP_CONFIG['v_host_config_dir']}invalid_nginx.conf"

        File.write(APP_CONFIG['webserver_config'], APP_CONFIG['webserver_config'] + 'syntaxerror')
      end

      it 'raises Exception' do
        expect { VirtualHostServiceWorker::NginxVHostWriter.config_valid? }.to raise_error
      end
    end
  end
end
